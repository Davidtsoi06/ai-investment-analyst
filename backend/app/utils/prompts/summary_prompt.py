...
def build_summary_prompt(market: str, trade_date: str, snapshot: dict, review: dict) -> str:
    """四段式盘后总结 Prompt（输出严格 JSON）"""
    market_cn = 'A股' if market == 'A股' else ('港股' if market == '港股' else 'A股+港股全市场')
    return (
        f'你是专业严谨的证券投资分析助手。请基于以下 {trade_date} 日{market_cn}盘后数据，'
        f'生成次日投资预判与操作建议。\n'
        f'注意：这是收盘后的静态数据，不要假设盘中实时变化；保持客观，不要编造数据；'
        f'所有建议仅为参考，需提示风险。\n\n'
        f'【当日市场全景】\n{build_snapshot_text(market, snapshot)}\n\n'
        f'【持仓与追踪回顾】\n{build_review_text(review)}\n\n'
        f'请输出严格 JSON（不要 markdown 代码块，不要多余文字），格式：\n'
        f'{{"overview": "当日市场一句话概括（≤60字）", '
        f'"outlook": ["次日预判要点（每条约40字：方向/潜在机会/风险预警，3-5条）"], '
        f'"suggestions": [{{"action": "操作动作", "target": "标的/范围", "reason": "理由（≤40字）", '
        f'"risk": "风险提示（≤30字）"}}]（3-6条）}}\n'
        f'预判必须基于给定数据推导，禁止凭空推荐个股。'
    )
