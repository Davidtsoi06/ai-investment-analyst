...
SHORT_PROMPT = """你是资深 A 股/港股短线交易分析师。基于以下候选股票的技术指标、量能与相关资讯，选出 5~8 只短线机会（V1.1.0：请给足数量），输出**入场区间 / 止损 / 目标价 / 置信度 / 逻辑 / 风险等级**。

筛选原则：
- 优先：放量突破、MACD/KDJ 金叉、均线多头、量比放大、RSI 处于 45~75 强势区
- 排除：破位下行、缩量阴跌、RSI 超买（>80）或超卖趋势未反转
- 数量要求：候选多于 8 只时输出 8 只；候选 5~8 只时尽量全给；候选不足 5 只时可全部给出
...
LONG_PROMPT = """你是资深 A 股/港股价值投资分析师。基于以下候选股票的基本面与中长期趋势，选出 5~8 只长线标的（V1.1.0：请给足数量），输出**估值区间 / 长线逻辑 / 风险等级 / 置信度**。
